...
def load_metadata_table(file_path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        if read_kwargs:
            logger.error(
                "Failed to parse %s as TSV (%s).", file_path, exc, exc_info=True
            )
            raise

        logger.info(
            "Standard CSV parsing failed for %s (%s); trying delimiter inference.",
            file_path,
            exc,
        )
...
def main():
    ...
    if not args.csv_file.is_file():
        parser.error(f"Metadata file not found: {args.csv_file}")
