...
# Import our custom encoders for patching
try:
    from timm_vit_encoder import TimmViTHugeEncoder
    CUSTOM_VIT_ENCODER_AVAILABLE = True
except ImportError:
    CUSTOM_VIT_ENCODER_AVAILABLE = False
...
def patch_ludwig_encoders():
    """Patch Ludwig's encoders to use our custom implementations."""
    patches_applied = []
    
    # Patch ViT encoder
    if CUSTOM_VIT_ENCODER_AVAILABLE:
        try:
            import ludwig.encoders.image.base
            ludwig.encoders.image.base.ViTEncoder = TimmViTHugeEncoder
            logger.info("✓ Successfully patched Ludwig's ViTEncoder with custom timm ViT Huge encoder")
            patches_applied.append("vit")
        except Exception as e:
            logger.error(f"Failed to patch Ludwig's ViTEncoder: {e}")
    else:
        logger.warning("Custom timm ViT encoder not available - skipping patch")
    
    # Patch Caformer encoder (register if available)
    if CUSTOM_CAFORMER_ENCODER_AVAILABLE:
        try:
            import ludwig.encoders.image.base
            ludwig.encoders.image.base.CaformerEncoder = CaformerEncoder
            logger.info("✓ Successfully patched Ludwig's CaformerEncoder with custom implementation")
            patches_applied.append("caformer")
        except Exception as e:
            logger.error(f"Failed to patch Ludwig's CaformerEncoder: {e}")
    else:
        logger.warning("Custom Caformer encoder not available - skipping patch")
    
    # FORCE PATCH: Bypass Ludwig's schema validation for custom encoders
    try:
        # Patch the schema validation to allow our custom variants
        import ludwig.schema.encoders.image.torchvision
        from ludwig.schema import schema_utils
        
        # Create a new schema that includes our custom variants
        class PatchedTVViTEncoderConfig(ludwig.schema.encoders.image.torchvision.TVViTEncoderConfig):
            model_variant: str = schema_utils.StringOptions(
                ...
                parameter_metadata=ludwig.schema.encoders.image.torchvision.ENCODER_METADATA["TVViTEncoder"]["model_variant"],
            )
        
        # Replace the original class
        ludwig.schema.encoders.image.torchvision.TVViTEncoderConfig = PatchedTVViTEncoderConfig
        
        logger.info("✓ FORCE PATCHED: Added caformer_b36_timm to allowed ViT model variants")
        patches_applied.append("schema_patch")
    except Exception as e:
        logger.error(f"Failed to patch Ludwig schema: {e}")
...
class LudwigDirectBackend:
    ...
    def prepare_config(
        ...
        # Check if this is our custom timm ViT Huge model
        if isinstance(raw_encoder, dict) and raw_encoder.get("custom_encoder", False):
            # Patch Ludwig's ViTEncoder with our custom implementation
            if patch_ludwig_encoders():
                logger.info(f"Using custom timm ViT Huge encoder for model: {model_name}")
            else:
                logger.warning(f"Failed to patch encoder for {model_name}, falling back to default ViT")
        
        if isinstance(raw_encoder, dict):
            # Remove custom_encoder flag from config as it's not a Ludwig parameter
            encoder_config = {
                **{k: v for k, v in raw_encoder.items() if k != "custom_encoder"},
...
def main():
    ...
    # Patch Ludwig schema for caformer support (only if caformer is selected)
    if args.model_name == "caformer_s18":
        patch_ludwig_schema_for_caformer()
